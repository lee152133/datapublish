...
public class UploadRemoteSXSInfoReq extends WSRequest {
	...
	/** 当前正在审讯的笔录 */
	private String remotetrialId;
	
	private List<SXMX_RemoteRoom> rooms;

	@Override
	protected void appendChildParamter(StringBuffer sb) throws Exception {
		if(rooms != null && rooms.size() > 0) {
			for(int i=0;i<rooms.size();i++) {
				SXMX_RemoteRoom r = rooms.get(i);
				...
				sb.append("<sxsxx>");
					...
					if(r.getRoomNumber() != null && !"".equals(r.getRoomNumber())) {
						sb.append("<roomNumber>").append("REMOTE-" + r.getRoomNumber()).append("</roomNumber>");
					} else {//暂定区域编号+审讯室id
						sb.append("<roomNumber>")
						.append(MessageFormat.format("{0}{1}{2}{3}", "REMOTE-" , r.getAreaid() , "-", r.getId()))
						.append("</roomNumber>");
					}
					sb.append("<name>").append(r.getName()).append("</name>");
					sb.append("<status>").append(r.getStatus()).append("</status>");
					if (r.getRecordid() != null) {
						sb.append("<recordid>").append(r.getRecordid()).append("</recordid>");
					} else {
	...
	public String getRemotetrialId() {
		return remotetrialId;
	}

	public void setRemotetrialId(String remotetrialId) {
		this.remotetrialId = remotetrialId;
	}

	public List<SXMX_RemoteRoom> getRooms() {
		return rooms;
	}

	public void setRooms(List<SXMX_RemoteRoom> rooms) {
		this.rooms = rooms;
	}
